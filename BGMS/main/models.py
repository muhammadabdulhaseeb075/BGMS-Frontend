...
class SiteDetails(models.Model):
    ...
    aerial = models.BooleanField(default=False, verbose_name="Aerial?")
...
class SiteGroup(models.Model):
    ...
    #if BurialGroundSite.objects.filter(id=burialgroundsite_id):
    burialgroundsite = models.ForeignKey(BurialGroundSite, on_delete=models.CASCADE, related_name="site_groups")
    #else:
    #    burialgroundsite = None
    objects = SiteManager()
    def __str__(self):
        #return str(self.burialgroundsite_id) + ';' + self.group.name
        try:
            #name = self.burialgroundsite.name
            #schema = self.burialgroundsite.schema_name
            #group = self.group_name
            #return name + ';' + schema + ';' + group
            return self.burialgroundsite.name + ';' + self.burialgroundsite.schema_name + ';' + self.group.name
        except:
            #print('Site matching ' + str(self.burialgroundsite_id) + ' does not exist.')
            return "No Site Matching " + str(self.burialgroundsite_id)
        #if self.burialgroundsite is not None:
        #    return self.burialgroundsite.name + ';' + self.burialgroundsite.schema_name + ';' + self.group.name
#        if hasattr(self, 'burialgroundsite'):
#            if hasattr(self.burialgroundsite, 'name'):
#                return self.burialgroundsite.name + ';'+ self.burialgroundsite.schema_name + ';' + self.group.name
...
class siteReferenceSettings(models.Model):
    ...
    burialgroundsite = models.ForeignKey(BurialGroundSite, on_delete=models.CASCADE, null=True)
